  ...
  return (
    ...
    >
      <button
        ...
        className={`relative flex h-full w-full flex-col justify-center gap-0.5 overflow-hidden rounded-lg border bg-surface px-3 py-2 text-left shadow-[0_1px_2px_rgba(42,36,25,0.06)] transition-all hover:shadow-[0_4px_12px_-2px_rgba(42,36,25,0.18)] ${
          isFemale ? "border-female/35" : "border-male/35"
        } ${selected ? "ring-2 ring-brand ring-offset-1" : ""} ${
          matched ? "ring-2 ring-gold" : ""
        } ${dimmed ? "opacity-30" : ""} ${person.is_deceased ? "bg-paper/70" : ""}`}
      >
        ...
        <span
          ...
          aria-hidden="true"
        />
        <span className="truncate pl-1.5 font-display text-sm leading-snug text-ink">
          {personName(person)}
        </span>
        {years && <span className="pl-1.5 text-xs text-ink-muted">{years}</span>}
        {person.urug && (
          <span className="truncate pl-1.5 text-[11px] text-gold">{person.urug}</span>
        )}
      </button>
    </div>
  );
